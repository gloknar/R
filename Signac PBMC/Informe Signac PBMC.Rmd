 ...
 * __Ratio reads in genomic blacklist regions__. El preciamente mencionado proyecto ENCODE ha publicado una lista negra de regiones cromosómicas que contiene un listado de las lecturas normalmente asociadas a ruido técnico. Cells with a high proportion of reads mapping to these areas (compared to reads mapping to peaks) often represent technical artifacts and should be removed. ENCODE blacklist regions for human (hg19 and GRCh38), mouse (mm10), Drosophila (dm3), and C. elegans (ce10) are included in the Signac package.
