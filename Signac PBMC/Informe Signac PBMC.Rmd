...
annotations <- GetGRangesFromEnsDb(ensdb = EnsDb.Hsapiens.v75) # Este paso es lento
...
Annotation(pbmc_Seurat@assays$peaks) <- annotations
```
 ...
 * __Total number of fragments (= reads) in peaks__: Las células con muy pocas lecturas se han secuenciado con poca profundidad y por tanto podemos eliminarlas del análisis. Células con muchas lecturas podrían ser _doublets_ (dos células secuenciadas como una), agregados de núcleos u otro tipo de artefactos.
 ...
 * __Ratio reads in genomic blacklist regions__. El proyecto ENCODE también ha publicado una [lista negra](https://github.com/Boyle-Lab/Blacklist) de regiones cromosómicas que consiste en un listado de las lecturas normalmente asociadas a ruido técnico. Las células con un un alto porcentaje de fragmentos que mapeen a dichos loci suelen ser ruido y deben eliminarse. Las listas negras de ENCODE actualmente disponibles en `Signac` son para genoma humano(hg19 y GRCh38), ratón (mm10), Drosophila (dm3), y _C. elegans_ (ce10).
...
```{r}
# DefaultAssay(pbmc_Seurat) # el Assay es el correcto: "peaks"
# Calculamos el nucleosome signal score por célula
pbmc_Seurat <- NucleosomeSignal(pbmc_Seurat)

# Calculamos el TSS enrichment score por célula
pbmc_Seurat <- TSSEnrichment(pbmc_Seurat)
...
```{r}
pbmc_Seurat$high.tss <- ifelse(pbmc_Seurat$TSS.enrichment > 2, 'High', 'Low')
