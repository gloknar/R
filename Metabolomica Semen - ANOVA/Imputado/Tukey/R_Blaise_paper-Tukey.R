...
which(colnames(dataset_semen) == "docosahexaenoylcholine")
dataset_semen$docosahexaenoylcholine
dataset_semen[,314]
colnames(dataset_semen)[314]
anyNA(dataset_semen[,314])  # NO HAY VALORES NA endocosahexaenoylcholine


which(colnames(dataset_semen) == "lysylleucine")
dataset_semen$lysylleucine
dataset_semen[,463]
colnames(dataset_semen)[463]
anyNA(dataset_semen[,463])  # NO HAY VALORES NA lysylleucine



which(colnames(dataset_semen) == "sucrose")
dataset_semen$sucrose
dataset_semen[,653]
colnames(dataset_semen)[653]
anyNA(dataset_semen[,653])  # NO HAY VALORES NA sucrose



# Graficos mierderos NAs
library(VIM)
library(naniar)
gg_miss_which(airquality)
gg_miss_fct(x = riskfactors, fct = marital) + labs(title = "NA in Risk Factors and Marital status")
gg_miss_fct(x = dataset_semen, fct = lysylleucine) + labs(title = "$ of NAs in lysylleucine")
objeto <- miss_case_summary(dataset_semen)
valoresNA <- objeto$case
metabolitosNA <- colnames(dataset_semen[,valoresNA])
c("lysylleucine", "sucrose", "docosahexaenoylcholine") %in% metabolitosNA


NAs_dataset_semen <- gg_miss_which(x = dataset_semen)
table(NAs_dataset_semen$data$value)
cosa2 <- NAs_dataset_semen$data
cosa2$value <- as.factor(cosa2$value)
cosa2[cosa2$value == "missing",]
cosa2$value


library(ggplot2)
gg_miss_fct(x = dataset_semen, fct = sucrose) + labs(title = "NA in algo")
??gg_miss_fct
...
# Mirar AUC con pROC (no fiable por lo que dije en inmputado BY)
# library(pROC)
# library(caret)
...
for (i in usar_metabs) { 
  ...
  # 
  # PD: con Resultados.$p_val_lm <= 0.05[,i] es la unica manera de que se lo trague el lm()
  # lm_fit <- lm(Resultados.$p_val_lm <= 0.05[,i] ~ GROUP + CENTRE + AGE + BMI + AGE:BMI, data = Resultados.$p_val_lm <= 0.05)
  ...
  Resultados. <- rbind(Resultados., resultados_metab)
}

# Ponemos bien los nombres de las columnas y las filas en la tabla de resultados
colnames(Resultados.) <- c("Metabolito", "p_val_lm", "p_val_Tukey")
rownames(Resultados.) <- Resultados.$Metabolito 
...
Resultados.$p_val_lm <- as.numeric(Resultados.$p_val_lm)
Resultados.$p_val_Tukey <- as.numeric(Resultados.$p_val_Tukey)


# Guardamos los p-valores corregidos por Tukey (Tukey controla el FWER)
# data.table::fwrite(Resultados., "Resultados_ANCOVA_Tukey.csv", sep = ",", 
#                    nThread = 2, row.names = T)
xlsx::write.xlsx(Resultados., "Resultados_ANOVA_Tukey.xls")



# Sacamos los boxplots de los metabs con un p-valor de Tukey <= 0.05
par(mar = c(2, 3, 2, 2))
# metabs_significativos <- which(colnames(dataset_semen_outliers) %in% rownames(Resultados.[(Resultados.$p_val_lm <= 0.05) & (Resultados.$p_val_Tukey <= 0.05),]))
...
for (i in c(1:length(metabs_significativos))) {
  png(filename = file.path("boxplots_metabs_Tukey_significativos",paste0(colnames(dataset_semen_outliers)[metabs_significativos[i]],"_boxplot_GROUP.png")), width = 658,
      height = 613)
  
  boxplot(dataset_semen_outliers[,metabs_significativos[i]] ~ dataset_semen_outliers[,7], las = 1, ylab = "", xlab = "",
          main = colnames(dataset_semen_outliers)[metabs_significativos[i]], col = c("cadetblue1", "brown2"))
  
  # Texto del p-val de la bondad del ajuste del modelo
  texto <-  paste0("p-val (lm) = ", format(Resultados.anova[indice_metab_tabla[i],1], digits = 3))
  mtext(text = texto, side = 3, line = -1, outer = F, at = 1.5)
  
  # Texto del p-val de diferencias entre grupos (Tukey)
  texto2 <-  paste0("p-val (Tukey) = ", format(Resultados.anova[indice_metab_tabla[i],2], digits = 3))
