...
El comando `RunUMAP` permite ejecutar un UMAP en Seurat. 10X Genomics recomienda tanto para t-SNE como UMAP buscar el mismo nº de dimensiones que las establecidas en el paso del PCA (7 en nuestro caso).
...
Seurat puede ayudarte a encontrar marcadores (*i.e.* genes diferencialmente expresados) que definen clusters. Por defecto, identifica marcadores positivos y negativos de un único cluster (especificado en el parámetro `ident.1`), comparado con el resto de células. El comando `FindAllMarkers` automatiza este proceso para todos los clusters, pero también puedes testar grupos de clusters entre ellos, o contra todas las células. (?)
...
```{r}
# Encontramos los marcadores para cada cluster comparado con el resto de células
