...
timing.comparisons <- rbind(timing.comparisons, data.frame(fxn = "JackStraw", time = as.numeric(end - 
    start, units = "secs"), strategy = "sequential"))

# Strategy = "multisession", 4 hilos
...
Para agrupar las células en clusters, a continuación aplicamos técnicas de  optimizacion de la modularidad tales como el [método de Louvain](https://sites.google.com/site/findcommunities/) (por defecto), o SLM ([SLM, Blondel *et al.*, Journal of Statistical Mechanics](http://dx.doi.org/10.1088/1742-5468/2008/10/P10008)), para agrupar células iterativamente con el objetivo de optimizar la función estándar de modularidad. El comando `FindClusters` implementa este procedimiento, y contiene el parámetro `resolution`(resolución) que establece la "granularidad" de los posteriores clusterings, y cuando tenemos un valor alto de `resolution`, más clusters generamos. 10X Genomics recomienda usar un `resolution` de entre 0,4 y 1,2 para datasets de single-cell de 3000 células. Para datasets con más de 3000, se recomienda usar valores más altos.

> Nota para mí: ver tema de modularidad en redes de coexpresión.  
"Networks with high modularity have dense connections between the nodes within modules but sparse connections between nodes in different modules. Modularity is often used in optimization methods for detecting community structure in networks. However, it has been shown that modularity suffers a resolution limit and, therefore, it is unable to detect small communities. Biological networks, including animal brains, exhibit a high degree of modularity"(https://en.wikipedia.org/wiki/Modularity_(networks))

Se puede comprobar a qué clúster pertenece cada célula con el comando `Idents(pbmc)`. En nuestro caso usamos `resolution = 0,5` dado que nuestro dataset es de menos de 3000 células polimorfonucleadas de sangre periférica.

```{r}
pbmc <- FindNeighbors(pbmc, dims = 1:12)
pbmc <- FindClusters(pbmc, resolution = 0.5, algorithm = 1) # algorithm = 1 para Louvain original; 2 para Louvain modificado; 3 para algoritmo SLM y 4 para algoritmo de Leiden
#?FindClusters
...
Tanto para el t-SNE como para el UMAP, 10X Genomics recomienda buscar el mismo nº de dimensiones como las establecidas en el apartado del PCA (en nuestro caso esta)
El comando `RunUMAP` permite ejecutar un UMAP en Seurat. 10X Genomics recomienda tanto para t-SNE como UMAP buscar el mismo nº de dimensiones que las establecidas en el paso del PCA (10 en nuestro caso).
...
Si crees que has avanzado bastante y no quieres perder tu progreso, puedes guardar tu objeto Seurat `pbmc` en un archivo `.rds` y así poder retomar tu trabajo en el futuro sin tener que volver a correr los algoritmos computacionalmente pesados de tu script. Guardar el objeto Seurat también te permite compartirlo fácilmente entre colaboradores.
...
Seurat puede ayudarte a encontrar marcadores que definen clusters vía expresión diferencial. Por defecto, identifica marcadores positivos y negativos de un único cluster (especificado en `ident.1`), comparado con todo el resto de células. `FindAllMarkers` utomatiza este proceso para todos los clusters, pero también puedes testar grupos de clusters vs cada uno o contra todas las células. (?)


El argumento `min.pct` requiere por defecto que la expresión de un gen sea detectada como mínimo en un 0.1% en cualquiera de los grupos celulares estudiados.*"a feature to be detected at a minimum percentage in either of the two groups of cells"*, y el parámetro `threst.test` requiere una característica ser diferencialmente expresada (en promedio) por una cantidad entre los dos grupos. Puedes poner ambos argumentos a 0, pero el algoritmo irá mucho más lento ()
```{r}
# ?Seurat::FindMarkers
```
...
# ?FindMarkers()
FindMarkers(pbmc, ident.1 = 1)
CellsByIdentities(pbmc)
```
