...
```{r}
# Nota: usar el paquete `future` para paralelizar NormalizeData, ScaleData,
# JackStraw, FindMarkers y quizás FindClusters (solo si hacemos clustering
# over multiple resolutions; nosotros usamos `resolution = 0.5`)
library(future)
plan(strategy = "multisession", workers = 4)
# Nota: la estrategia "multiprocess" de la guía original está obsoleta. En su lugar se usa la estrategia "multicore" (no soportada en Windows) o "multisession" (soportada en Windows)
...
# Strategy = "multicore", 4 hilos
plan("multicore", workers = 4)
start <- Sys.time()
pbmc <- JackStraw(pbmc, dims = 5)
end <- Sys.time()
timing.comparisons <- rbind(timing.comparisons, data.frame(fxn = "JackStraw", time = as.numeric(end - 
    start, units = "secs"), strategy = "multicore"))
