...
library(Seurat)
# remotes::install_version("Seurat", version = "3.2") # Seurat V3.2 no es compatible con la ultima version de spatstat, o sea, la 2.0-1
...
mouse <- FindVariableFeatures(mouse)
hvg.human <- rownames(head(human@meta.data, 1000)) # Coge el nombre de las primeras 1000 celulas, parece
hvg.mouse <- rownames(head(mouse@meta.data, 1000)) # ?????mirar eso de hvg.info higly variable gen supongo
hvg.union <- union(hvg.human, hvg.mouse) # Concatena las dos listas de strings
```

```{r}
# now we scale the data, adding in the individual (stored in orig.ident)
# as a regression covariate
mouse <- ScaleData(mouse, vars.to.regress = "orig.ident", genes.use = hvg.union) # Mirar genes.use
