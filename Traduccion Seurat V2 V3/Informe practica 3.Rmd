...
print(pancreas.combined@reductions$cca, dims = 1:5, nfeatures = 15)


# Graficamos ambos datasets en la proyección de dimensionalidad reducida "cca"
DimPlot(pancreas.combined, group.by = "species", reduction = "cca")
```


Corremos el PCA para poder ver el t-SNE:

```{r}

# Seurat V2 usa `CalcVarExpRatio`; Seurat V3 usa... mirar
...
# respectivamente.
# `RunPCA` en Seurat V3 no usa `use.imputed` ni `do.print`. `assay.type` =
# `assay`. `pc.genes` = `features`. `do.print` = `verbose`
pancreas.combined <-RunPCA(pancreas.combined , features = hvg.union, npcs = 20,
                           rev.pca = FALSE,
                           weight.by.var = TRUE, ndims.print = 1:5,
                           nfeatures.print = 30, reduction.name = "pca",
...
Corremos el t-SNE y lo visualizamos:
