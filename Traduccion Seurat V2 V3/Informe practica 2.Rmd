...
`TSCAN` combina el clustering con el análisis de pseudotiempo. Se agrupan las células usando el comando `exprmclust`, el cual se basa en una mezcla de distribuciones normales. Seguidamente construye un árbol del menor tamaño posible ( _minimum-spanning-tree_ o MST) que conecte los clusters. La rama principal del árbol será la que conecte el mayor nº de clusters, y es la rama que se usa para determinar el pseudotiempo.
...
```{r}
# El último cúster (nº 10), o sea el último tramo del pseudotiempo según TSCAN,
...
Observamos que al igual que el PCA, `TSCAN` nos devuelve la trayectoria al revés, y es más, dicha trayectoria es similar a la obtenida en el PCA. Ello se debe a que TSCAN es un método de inferencia de trayectoria basado en el PCA. En vista del gráfico obtenido, no podemos asegurar que `TSCAN` funcione mejor que el ordenamiento que hicimos mediante las coordenadas de la 1ª componente principal.
