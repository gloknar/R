...
```{r, warning=F}
# En este caso tenemos que usar scale_y_reverse(), seguramente porque
# internamiente los comandos `plot_switch_times()` y `plot_peak_times()` hacen
# un flip de las coordenadas

# La leyenda está al revés
plot_switch_times(oui = oui_vb) + theme_classic() + 
  scale_y_reverse() + theme(legend.position = "top")

data(oui)
View(oui_vb)
oui_vb$fit@inits$k <- -oui_vb$fit@inits$k # Esto deberia funcionar...
jajaja <- extract(oui_vb$fit, "k")$k
jajaja <- -jajaja
View(oui_vb$fit)
