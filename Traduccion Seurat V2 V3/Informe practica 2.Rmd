...
Ahora que hemos identificado 5 estados celulares metaestables, procedemos a identificarlos mediante su comparación con la verdad experimental:
...
* Efectivamente, el algoritmo es sensible a las probabilidades _a priori_ de los genes. Resulta muy beneficial para el algoritmo tener muy claro el funcionamiento de los biomarcadores que le proporciones y elegir en consecuencia unos valores de _a prioris_ elevados y de signo correcto ( _i.e._ poner las _a priori_ de los genes monotónicos down en positivo, y viceversa para los genes up confunde al algoritmo y no puede clasificar correctamente las células ni averiguar cuándo se expresan o se silencian los genes).
...
```{r, fig.height = 9.5, eval = F}
# Genes de la etapa inicial(excepto Nanog). Comienzan activos y se desactivan a
# medida que progresa el pseudotiempo (de ahí lo de "down").
ouija_markers_down <- c("Hoxa7", "Hoxa1", "Hoxd4", "Hoxb2", "Fgf8", "Egfr", "Bmp5", "Bmp15") 
...
# Mirar: https://www.nature.com/scitable/topicpage/hox-genes-in-development-the-hox-code-41402/#
# rownames(log_deng)
# grep("Hox", rownames(log_deng), value = T)
```
...
`ouija` responde a estas preguntas en términos de un test de hipótesis Bayesiana de si la diferencia en los tiempos de regulación, ya sea _switch time_ o _peak time_, es significativamente distinta de 0. Esto se comprueba con el comando `gene_regulation`.
...
Si comparamos dos genes monotónicos de la etapa temprana, Egfr y Dazl, el primero se desactiva/infraexpresa antes que el segundo. 
