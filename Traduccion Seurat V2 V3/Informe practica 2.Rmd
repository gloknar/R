---
title: "Informe práctica 2 - Métodos para inferencia de la trayectoria en procesos biológicos"
author: "Adam Casas"
...
```{r}
# Leemos los datos
deng_SCE <- readRDS(file = "./Cosas accesorias Informe 2/deng-reads.rds")
...
```{r, eval = F, echo = F}
# Aqui guardo todo lo del objeto de tipo `SingleCellEexperiment`, imaginatelo
# como una carpeta del ordenador
names(colData(deng_SCE))
colnames(colData(deng_SCE)) # es lo mismo que el comando anterior
```
...
10 años después, [Angerer _et al._](https://academic.oup.com/bioinformatics/article/32/8/1241/1744143) aplicaron exitosamente el concepto de los mapas de difusión al análisis de datos scRNA-seq para crear así un paquete de R llamado `destiny`. Para más información sobre el paquete y sus viñetas, véase la página correspondiente del [ICB](https://www.helmholtz-muenchen.de/icb/research/groups/marr-lab/software/destiny/index.html).
...
`SLICER` es un algoritmo para construir trayectorias que describen cambios en la expresión génica durante un proceso biológico secuencial (al igual que `Monocle` y `TSCAN`). `SLICER` está diseñado para capturar cambios de expresión génica no lineales, seleccionar automáticamente aquellos genes que participen en el proceso biológico y detectar _branches_/caminos alternativos y _loops_ en la trayectoria ([Welch _et al._, 2016](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-016-0975-3)). El paquete de `SLICER` está disponible en [GitHub](https://github.com/jw156605/SLICER) y se puede instalar mediante las utilidades del paquete `devtools`. `SLICER` asume que la mayoría de la variación observada en el dataset proviene de un proceso biológico secuencial con capacidad de ramificarse (tomar caminos paralelos), por lo que se debe tener en cuenta si dicha suposición es acertada para el dataset en cuestión.


Para comenzar el _workflow_, emplearemos la función `select_genes` de `SLICER` para seleccionar automáticamente los genes que se usarán para construir la trayectoria celular. Dicha función usa la _"neighbourhood variance"_ para identificar los genes que varían suavemente (en lugar de aquellos que fluctúan aleatoriamente) en las células estudiadas. A continuación, determinaremos mediante el comando `select_k` el nº de vecinos cercanos o _nearest neighbours_ que devuelva un _embedding_ o embebimiento (construcción/proyección en un espacio de dimensionalidad reducida) lo más parecido a una trayectoria. Finalmente estimaremos el embebimiento localmente lineal (LLE) de las células mediante el comando `lle` del paquete homónimo. 

Téngase en cuenta que:

 * Una buena práctica para determinar el nº óptimo de `k` para el LLE es probar distintos valores de `k` y examinar visualmente dichas las proyecciones.
 
 * En algunos casos (sobretodo cuando el dataset cuenta con muchos estados celulares), usar un LLE con más de 2 dimensiones puede mejorar los resultados.
    

```{r, warning = F}
# Para seleccionar los genes en `SLICER`, usamos la matriz de conteos
# logarítmica traspuesta. Ídem para seleccionar el parámetro `k` del LLE
...
Ahora que hemos calculado el LLE, podemos construir un grafo basado en _k-nearest neighbours_ (k-NN) que sea completamente conectado ( _fully connected_, como se suele decir en el ámbito de los perceptrones multicapa) mediante el comando `conn_knn_graph`. En el grafo a continuación se representan las células como círculos naranjas con sus IDs en azul. Mostramos a continuación el grafo obtenido con 10 vecinos. El parámetro `k` del grafo se suele establecer en $5$, aunque a veces variar ligeramente su valor puede mejorar los resultados.
...
* Identifica estados metaestables a lo largo de la trayectoria, _i.e._ tipos celulares.
...
# la solución 2 (los bucles for no son eficientes en R)
...
Adicionalmente, con `ouija` podemos detectar __estados celulares metaestables__ usando matrices de consistencia. Los estados celulares metaestables son un sinónimo de tipos celulares, ya que la idea detrás del nombre es que a medida que las células se diferencian durante el proceso biológico, pueden mantener un estado estable en un tramo de la trayectoria, lo que contaría como un tipo celular. Esto puede ser útil para detectar nuevos tipos celulares en procesos de diferenciación más o menos conocidos, como por ejemplo en la diferenciación de las células troncales hematopoyéticas ([Ceredig _et al._, 2009](https://pubmed.ncbi.nlm.nih.gov/19282853/)). 

Para identificar estos tipos celulares metaestables, primero tenemos que computar la matriz de consistencia con el comando `consistency_matrix`, y acto seguido agrupar las células con `cluster_consistency`. Tras ello, podemos computar el pseudotiempo con el comando `map_pseudotime` y visualizar cómo se suceden dichos estados celulares a lo largo del pseudotiempo. Nótese que comprobé posteriormente que `ouija` también nos devuelve el pseudotiempo al revés, así que lo corregiremos aquí para ahorrarnos futuros quebraderos de cabeza.
...
Ahora que hemos identificado 4 estados celulares metaestables, procedemos a identificarlos mediante su comparación con la verdad experimental:
...
En vista del gráfico, anotaremos esos 4 estados metaestables como “zygote/2 cell”, “4/8/16 cell”, "early/mid blast" y “late blast”.
...
* `TSCAN` es un método basado en MST o Minimum Spanning Tree, y es conceptualmente muy parecido a `Monocle`. Además, hace uso de PCA para reducir la dimensionalidad de los datos y Mclust para agrupar las células en un nº óptimo de clusters, determinado automáticamente usando el criterio de información bayesiano (= BIC o criterio de Schwarz). `TSCAN` calcula un MST que pasa por el centroide de todos los clusters y de ahí infiere la trayectoria como el camino más largo a través del árbol (Monocle también busca la ruta más larga en un MST). Por último, las células son proyectadas al punto más cercano de la trayectoria, creando el orden final de las mismas. `TSCAN` es además uno de los pocos métodos completamente no supervisado, ya que al igual que `Monocle`, determina automáticamente los clusters de partida y de fin de la trayectoria. `TSCAN` es fácil de usar dado que determina todos sus parámetros automáticamente. Si bien la trayectoria inferida por `TSCAN` es más fiable que la de `Monocle` (puesto que `TSCAN` agrupa primero las células en clusters antes de calcular el camino más largo en el MST), dicha trayectoria sigue sin ser robusta, dado que ligeros cambios en la localización de los centroides puede provocar grandes cambios en la susodicha trayectoria inferida.
...
* `Monocle` es un método para detectar trayectorias ramificadas. Primero detecta genes diferencialmente expresados en las poblaciones celulares de interés y luego aplica un ICA sobre dichos genes para reducir la dimensionalidad de los datos (esto se hace así porque el ICA no escala bien con muchos genes). Después se calcula un MST sobre las células y el camino más largo a través del mismo (nótese que al calcular el MST sobre células individuales y no sobre clusters como hace `TSCAN`, este algoritmo es sensible al ruido). Cada célula se proyecta al punto más cercano de la trayectoria inferida. Al ejecutar el ICA sólo sobre genes diferencialmente expresados, se introduce un sesgo en los resultados.


<br>

Nótese que los métodos actuales de inferencia de trayectoria raramente son capaces de modelar caminos alternativos que convergan en un mismo destino celular. También asumen que siempre hay una trayectoria en los datos, aunque no siempre sea el caso, por lo que en esas situaciones se recomienda emplear métodos de clustering y/o visualización alternativos tales como `SPADE`, `viSNE` o `FlowSOM`.

Para finalizar, conviene tener en cuenta que las trayectorias generadas por estos métodos no dejan de ser hipótesis, las cuales deben ser validadas experimentalmente. 
...
La mayoría de los paquetes vistos en esta práctica cuentan con herramientas para visualizar la expresión génica a lo largo del pseudotiempo. No obstante, `SLICER` carece de estas herramientas, por lo que es conveniente suplir esa carencia con las utilidades del paquete `scater`. Seguir indivualmente el rastro de ciertos genes resulta útil para identificar aquellos que tengan un rol importante en el proceso de diferenciación celular estudiado. Ilustraremos el procedimiento usando los genes __Rhoa__, __Hoxa1__ y __Carm1__.

Dado que hemos añadido el pseudotiempo computado por cada algoritmo estudiado al bolsillo `colData` del objeto `deng_SCE`, podemos usar `scater` para investigar de manera visual la relación entre expresión génica, poblaciones celulares y pseudotiempo en nuestro dataset. Para ver en más detalle las utilidades que incorpora `scater`, véase su [viñeta online](https://bioconductor.org/packages/release/bioc/vignettes/scater/inst/doc/overview.html).


__Rhoa:__

De acuerdo a la información disponible en [GeneCards](https://www.genecards.org/cgi-bin/carddisp.pl?gene=RHOA) y [MGI](http://www.informatics.jax.org/marker/MGI:1096342), el gen Rhoa:

* Se localiza en el cromosoma 9 en ratón

* Codifica para una GTPasa de la familia Rho, la cual actúa como interruptor molecular en cascadas de señalización

* Promueve la reorganización del citoesqueleto, alterando así la forma, motilidad y adhesión celular. Además, participa en la citocinesis

* Se expresa en el desarrollo embrionario



<br>

Visualizamos la expresión génica de __Rhoa__ a lo largo de las trayectorias generadas:

__PCA:__
```{r}
plotExpression(deng_SCE, features = "Rhoa", x = "PC1", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```
...
De acuerdo a la información disponible en [GeneCards](https://www.genecards.org/cgi-bin/carddisp.pl?gene=HOXA1) y [MGI](http://www.informatics.jax.org/marker/MGI:96170), el gen Hoxa1:

* En ratón, su locus se halla en el cromosoma 6

* Codifica para un factor transcripción que se une a motivos ( _motifs_) de ADN

* Regula procesos de desarrollo tales como la formación del tronco encefálico, oído interior y exterior, nervio VI ( _abducens_) y el sistema cardiovascular

* El producto génico podría estar involucrado en la generación del tejido anterior-posterior (delante-arriba) del metencéfalo, el cual se diferenciará en cerebelo, puente de Valorio y los nervios craneales V (trigeminal o trigémino), VI ( _abducens_), VII (facial) y VIII (auditivo).

* Codifica para dos transcritos de ARNm y produce 2 isoformas (aunque sólo una contiene la región del homeodominio)

* Se expresa en las fases tempranas del desarrollo embrionario


<br>

Visualizamos la expresión génica de __Hoxa1__ a lo largo de las trayectorias generadas:

__PCA:__
```{r}
plotExpression(deng_SCE, features = "Hoxa1", x = "PC1", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__TSCAN:__
```{r}
plotExpression(deng_SCE, "Hoxa1", x = "pseudotime_order_tscan", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__Mapa de difusión:__
```{r}
plotExpression(deng_SCE, "Hoxa1", x = "pseudotime_diffusionmap", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__SLICER:__
```{r}
plotExpression(deng_SCE, "Hoxa1", x = "pseudotime_slicer", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__Ouija:__
```{r}
plotExpression(deng_SCE, "Hoxa1", x = "pseudotime_ouija", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```
...
De acuerdo a la información disponible en [Genecards](https://www.genecards.org/cgi-bin/carddisp.pl?gene=CARM1) y [MGI](http://www.informatics.jax.org/marker/MGI:1913208), el gen Carm1:

* Se localiza en el cromosoma 9 en ratón

* Codifica para una enzima que metila argininas de histonas y proteínas asociadas a la cromatina

* Participa en procesos de regulación de la expresión génica a través del remodelado de cromatina y la regulación de la transcripción, del splicing alternativo de pre-ARNm y de la estabilidad del ARNm. También silencia los genes de la familia CREB
...
Visualizamos la expresión génica de __Carm1__ a lo largo de las trayectorias generadas:

__PCA:__
```{r}
plotExpression(deng_SCE, features = "Carm1", x = "PC1", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__TSCAN:__
```{r}
plotExpression(deng_SCE, "Carm1", x = "pseudotime_order_tscan", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__Mapa de difusión:__
```{r}
plotExpression(deng_SCE, "Carm1", x = "pseudotime_diffusionmap", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__SLICER:__
```{r}
plotExpression(deng_SCE, "Carm1", x = "pseudotime_slicer", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```


__Ouija:__
```{r}
plotExpression(deng_SCE, "Carm1", x = "pseudotime_ouija", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```
...
<summary> __Ejercicio 2.6?: Para nuestro dataset, ¿qué algoritmos de los aquí estudiados riden mejor que el PCA a la hora de inferir el pseudotiempo?__</summary>

<br>

__Solución:__

Nos encontramos ante una respuesta compleja, y es que al igual que sucede en _Machine Learning_, no hay un algoritmo que dé el mejor resultado en todos los casos, sino que cada uno de los algoritmos estudiados destaca en algo.

* Para etapas tempranas del desarrollo (de "zy" a "4 cell"), `destiny` es el método que mejor separa las etapas, y en su orden correcto.

* Para las etapas "8 cell" y "16 cell", `SLICER` es el algoritmo que mejor las separa, pero les asigna un orden incorrecto, pues según `SLICER`, la etapa de 16 células se sucede antes que la etapa de 8 células. Por otro lado, `TSCAN` ordena correctamente ambas etapas, y las separa relativamente bien (exceptuando unas pocas células de la etapa "8 células") 

* Para las 3 etapas del blastocisto, el PCA es el método que mejor las separa del resto de etapas, aunque si nos centramos en esas 3 etapas, le cuesta discernir cuándo una célula es del "early blast", "mid blast" o "late blast". En ese aspecto, `SLICER` logra distinguir con mejores resultados las 3 etapas de blastocisto entre sí, si bien no hay ningún algoritmo que produjese una separación limpia entre esas 3 etapas.

* Finalmente, comentar que si bien `SLICER` no es perfecto, parece producir los clusters más compactos en general.



<br>

A continuación se recopilan los resultados obtenidos por los métodos aquí estudiados:

__PCA__:
![](./Cosas accesorias Informe 2/ordenamientos/ordenamiento_PCA.png)




__TSCAN__:
![](./Cosas accesorias Informe 2/ordenamientos/ordenamiento_TSCAN.png)



__ouija__:
![](./Cosas accesorias Informe 2/ordenamientos/ordenamiento_ouija.png)


__SLICER__:
![](./Cosas accesorias Informe 2/ordenamientos/ordenamiento_SLICER.png)


__destiny__:
![](./Cosas accesorias Informe 2/ordenamientos/ordenamiento_destiny.png)
