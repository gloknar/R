...
```{r}
# Biomarcadores
ouija_markers_down <- c("Hoxa1", "Creb3", "Carm1")
ouija_markers_transient <- c("Zscan4b", "Foxa1") 
ouija_markers_up <- c("Rhoa", "Sox2")
ouija_markers <- c(ouija_markers_up, ouija_markers_down, ouija_markers_transient)
...
# Con `grep` (RegEx) obtenemos los índices de los k.1-k.5 y los visualizamos con
# `print()`
indices <- grep("k.", names(oui_vb$fit@sim$samples[[1]])); print(c(min(indices), max(indices)))
...
plot_expression(oui_vb) + scale_x_reverse()

plot_switch_times(oui_vb)  + scale_y_reverse()
```
...
* Promueve la reorganización del citoesqueleto, alterando así la forma, motilidad y adhesión celular
...
* Participa en procesos de regulación de la expresión génica a través del remodelado de cromatina y la regulación de la transcripción, del splicing alternativo de pre-ARNm y de la estabilidad del ARNm. También silencia a los genes de la familia CREB (véase gráfico de los tiempos de regulación)
...
plotExpression(deng_SCE, features = "Creb3", x = "pseudotime_ouija", colour_by = "cell_type2", show_smooth = T, show_violin = F)

plotExpression(deng_SCE, features = "Creb1", x = "pseudotime_ouija", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```
...
# T test?
# Anova?
# ¿Correción de p-valor con Benjamini- Hochberg?
# ¿Correción de p-valor con Bonferroni?
