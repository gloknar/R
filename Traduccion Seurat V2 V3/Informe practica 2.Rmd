...
library("lle")
slicer_traj_lle <- lle(t(log_deng[slicer_genes,]), m = 3, k)$Y # Importante usar esta notación, si no, no funciona el comando `conn_knn_graph()`
...
plotReducedDim(deng_SCE, dimred = "LLE", colour_by = "cell_type2", ncomponents = 3) +
...
```{r}
# Construimos el grafo con k = 10 vecinos
...
# Graficamos en 3D (opcional)
library(plotly)
p <- plot_ly(data = data.frame(slicer_traj_lle), type = "surface")
...
ouija_markers_up <- c("Creb3", "Gpx4", "Krt8", "Elf5", "Eomes", "Cdx2", "Tdgf1", "Gdf3") 
...
Ahora que hemos identificado 5 estados celulares metaestables, procedemos a identificarlos mediante su comparación con la verdad experimental (curiosamente, Pedro decetó sólo 4):
...
PCA:
```{r}
plotExpression(deng_SCE, "Rhoa", x = "PC1", colour_by = "cell_type2", show_smooth = T, show_violin = F)
```
