...
```{r, warning=F}
# Graficamos los switch times
plot_switch_times(oui_vb)
# Graficamos los peak times
plot_peak_times(oui_vb)
```

Se aprecia que igual que en el gráfico anterior, la trayectoria está invertida. Además, el gráfico que devuelve `plot_switch_times()` indica la regulación al revés (El gen Fgf8 se desactiva al comienzo de la trayectoria, aunque en el gráfico indique que se desactiva, ídem para el gen Egfr). Procedemos a corregir ambos gráficos inviertiendo el eje X de ambos gráficos y la leyenda del gráfico `plot_switch_times`, teniendo en cuenta que:
...
# Solución elegante:
#
# Obtenemos los índices que contienen los objetos k.1 - k.16 en el objeto
# `oui_vb$fit@sim$samples`. Hacemos esto porque el comando `plot_switch_times()`
# obtiene la información sobre regulación génica de ahí.
