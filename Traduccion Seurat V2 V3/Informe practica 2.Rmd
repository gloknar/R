...
* LLE con k bajos pasa esto

* LLE con k altos pasa esto otro.

* A `conn_knn_graph()` le pasa esto o lo otro al variar k




__LLE con K = 5, grafo a K = 10__

```{r}
set.seed(1)
k <- 5

slicer_traj_lle <- lle(t(log_deng[slicer_genes,]), m = 2, k)$Y
...
ggplot(as.data.frame(colData(deng_SCE)), aes(x = pseudotime_slicer, y = cell_type2, colour = cell_type2)) + 
  geom_quasirandom(groupOnX = F) + xlab(" \"Pseudotiempo\" (orden celular) ") + ylab("etapa de desarrollo") +
  theme_classic() + ggtitle("LLE con k = 5; grafo con k = 10") + theme(plot.title = element_text(hjust = 0.5))
```
