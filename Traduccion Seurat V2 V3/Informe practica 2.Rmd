...
Los paquetes estudiados cuentan con herramientas para visualizar la expresión génica a lo largo del pseudotiempo. Seguir el rastro de genes individuales resulta útil para identificar aquellos genes que tengan un rol importante en el proceso de diferenciación celular estudiado. Ilustraremos el procedimiento usando los genes __Rhoa__ y __Hoxa1__[2](https://www.genecards.org/cgi-bin/carddisp.pl?gene=HOXA1&keywords=hoxa1).


Hemos añadido el pseudotiempo computado por cada algoritmo estudiado al bolsillo `colData` del objeto `deng_SCE`. Por tanto, podemos usar el paquete `scater` para investigar de manera visual la relación entre expresión génica, poblaciones celulares y pseudotiempo. El paquete `scater` es especialmente útil en el caso de paquetes como `SLICER`, los cuales carecen de herramientas para graficar expresión génica.


Compara los gráficos de violín y los de ouija del comando `plot()`, son muy parecidos...

```{r}
# Violín
plotExpression(deng_SCE, features = "Rhoa", x = "cell_type2", colour_by = "cell_type2")
plotExpression(deng_SCE, features = "Hoxa1", x = "cell_type2", colour_by = "cell_type2")
...
```{r}
# Genes de la etapa inicial. Comienzan activo y se desactiva a medida que progresa
# el pseudotiempo (de ahí lo de "down").
ouija_markers_down <- c("Hoxa1", "Rhoa")

# Gene de la etapa intermedia. Comienzan inactivos, se activan un rato y
# vuelven a desactivarse hacia el final de la trayectoria. Presentan una
# activación transitoria
ouija_markers_transient <- c("Zscan4b", "Foxa1") 

# Gen de la etapa tardía. Comienza inctivo y se activan a medida que progresa el
# pseudotiempo (de ahí lo de "up").
ouija_markers_up <- c("Creb3") # Mirar gen Sox2
...
* Ver información en __Genecards__ y expresión experimental en _[The Human Protein Atlas](https://www.proteinatlas.org/search/embryo)_.

https://www.proteinatlas.org/search/Rhoa

* Participa en procesos de transducción de señales intracelulares ([1](https://www.genecards.org/cgi-bin/carddisp.pl?gene=RHOA)
...
Visualización de la expresón génica de __"Rhoa"__ a lo largo de las trayectorias generadas:
...
* Forma parte del clúster A del cromosoma 7

* Codifica para un factor transcripción que se une a motivos ( _motifs_) de ADN

* Regula procesos de desarrollo como formación del brainstem tronco encefálico o _brainstem_, oído interior y exterior, nervio VI ( _abducens_) y el sistema cardiovascular
...
* Codifica para dos transcritos de ARNm, y produce 2 isoformas (aunque solo una contiene la región del homeodominio)
...
Buscar expresión experimental de HoxA1 en _[The Human Protein Atlas](https://www.proteinatlas.org/search/embryo)_.

https://www.proteinatlas.org/search/hoxa1
...
Visualización de la expresón génica de __"Hoxa1"__ a lo largo de las trayectorias generadas:
