...
Un análisis típico en estos casos es la detección de genes expresados diferencialmente a lo largo de la trayectoria. Monocle 3 permite afrontar este problema de dos maneras:

* Usando modelos de regresión mediante `fit_models()` para evaluar si la expresión de un gen varía en función de covariables tales como tratamiento, tiempo, edad, etc...

* Mediante el análisis de autocorrelación del grafo usando `graph_test()` (función paralelizable en UNIX pero no en Windows) para encontrar genes que varían entre clusters o a lo largo de la trayectoria.

<br>

La segunda manera consiste en un novedoso método basado en el test I de Moran. La I de Moran I es un estadístico que mide autocorrelaciones espaciales multidimensionales y multidireccionales. Esta medida indica si señales espaciales colindantes (en este caso, las células del grafo y sus vecinas) tienen valores similares (=niveles de expresión similares u opuestos para el gen testeado). Si bien la correlación de Pearson y la I de Moran devuelven medidas entre -1 y 1, la interpretación de la I de Moran es ligeramente distinta: +1 significa que células adyacentes tienen una expresión génica idéntica (imagen inferior, panel derecho); 0 indica que no hay correlación alguna (panel central) y -1 significa que las células vecinas están inversamente correlacionadas (panel izquierdo).
...
En esta práctica nos centraremos en el análisis de autocorrelación espacial, puesto que [Cao _et al._(2017)](https://science.sciencemag.org/content/357/6352/661) demostraron su exitosa aplicabilidad a datasets de scRNA-seq para el análisis de expresión diferencial. Según busquemos variaciones entre clusters o a lo largo de la trayectoria, tendremos que cambiar ligeramente el código empleado. Nosotros mostraremos cómo buscar genes que varían a lo largo de la trayectoria:


```{r, message = F}
# Expresión génica diferencial a lo largo de la trayectoria (test de la I de
# Moran)
pr_test_res <- graph_test(c3,  neighbor_graph = "principal_graph", cores = 1, verbose = F)
```
...
```{r}
# 'pr_test_res' returns a data frame containing the p values and q-values 
...
# Ver qué hace aquí
library("dplyr")
X <- dplyr::add_rownames(pr_test_res) %>%
...
sig <- nrow(subset(pr_test_res, q_value < 0.01))

plot_cells(c3, genes = Y$gene_short_name[1], alpha = 0.7)


plot_cells(c3, genes = Y$gene_short_name[2], alpha = 0.7)
```


Al igual que hicimos en la práctica 2 con `scatter` y `ouija`, Monocle 3 1.0.0 te permite visualizar cómo cambia la expresión génica a lo largo del pseudotiempo:

```{r, eval = F}
AFD_genes <- c("HMGA2")
AFD_lineage_cds <- c3[rowData(c3)$gene_short_name %in% AFD_genes,
                       colData(c3)$cell.type %in% c("AFD")]


plot_genes_in_pseudotime(c3, color_cells_by = "pseudotime")
