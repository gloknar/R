...
# Cargamos en memoria el dataset de Seurat 3  
...
Para los objetos de tipo `CellDataSet`, podemos acceder a su información con los métodos `pData()`, `fData()` y `exprs()` de Monocle 3:

```{r}
# Vemos los nombres de las columnas de los metadatos del objeto seurat. Estos
# son los metadatos de las células
colnames(pData(c3))
...
En este paso estimamos los factores de tamaño para cada célula (https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1874-1  “size factors” ) y la dispersión (=varianza) de los genes en el objeto `c3`. Monocle 3 realiza estas y otras operaciones con el paquete `DelayedArray`, lo que le permite escalar con millones de células. Dicho paquete divide las operaciones en bloques para prevenir el consumo excesivo de RAM del ordenador. Puedes controlar el tamaño de cada bloque y los mensajes que se devuelven por consola con los siguientes comandos:


```{r}
# Selecciona TRUE para ver en consola el progreso de algunas operaciones de Monocle 3
...
Ahora procedemos a computar la proyección no lineal de los datos en t-SNE o UMAP (por defecto en Monocle 3 1.0.0) con el comando `reduce_dimension()`.
...
plot_cells(c3, genes = "GBX2", group_label_size = 6, cell_size = 1) # Biomarcador de early epiblast

plot_cells(c3, genes = "GATA3", norm_method = "log",
           group_label_size = 6, cell_size = 1) # Biomarcador del hipoblasto
