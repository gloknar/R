...
Para los objetos de tipo `CellDataSet`, podemos acceder a su información con los métodos `pData()` y `fData()` de Monocle 3:
...
En este paso estimamos los factores de tamaño para cada célula (????) y la dispersión de los genes en el objeto `c3`. Monocle 3 realiza estas y otras operaciones con el paquete `DelayedArray`, lo que le permite escalar con millones de células. Dicho paquete divide las operaciones en bloques para prevenir el consumo excesivo de RAM del ordenador. Puedes controlar el tamaño de cada bloque y los mensajes que se devuelven por consola con los siguientes comandos:
