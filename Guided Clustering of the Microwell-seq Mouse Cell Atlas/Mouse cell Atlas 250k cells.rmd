...
print(mca@reductions$pca, dims = 1:5, nfeatures = 5)
```
...
## Clustering de las células

Ahora agrupamos las células mediante la técnica de clustering basadas en grafos de Seurat.


```{r}
mca <- FindNeighbors(mca, reduction = "pca", dims = 1:75, nn.eps = 0.5)
mca <- FindClusters(mca, resolution = 1.4, n.start = 10) # resolution > 1.2 ya que el dataset > 3000 células
# ?FindClusters
```
