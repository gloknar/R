...
hist(x = metadatos$peak_region_fragments, 
     main = "Distribución de los peak region fragments", col = "turquoise")
...
```{r}
MB_Signac <- TSSEnrichment(MB_Signac, fast = F) # Si usas el argumento fast = T, no puede ejecutar el TSSPlot, imagino que dihco argumento omite la matriz necesaria para dicho comando 
```
...
MB_Signac <- FindClusters(MB_Signac, verbose = FALSE, algorithm = 3) # Desactiva el paralelizado de `future` para evitar errores aleatorios
