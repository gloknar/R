...
La precisión en el set de entrenamiento es ligeramente mejor que en la de test (~15% de diferencia), por lo que puede que haya overfitting, pero no estamos seguros.
...
A continuación hacemos el __contraste para la igualdad de varianzas__ de los índices Kappa. Como son 4 poblaciones __normales y dependientes__, usaremos el __test de esfericidad de Mauchly__, cuya hipótesis nula H0 establece que se cumple la condición de homocedasticidad en las muestras.
...
# library("ez")
# ?ezANOVA
# anova.12398 <-ezANOVA(data = matriz_anova_kappa, detailed = T, wid = y, dv = V1, within = V2)
# matriz_anova_kappa <- cbind2(matriz_anova_kappa, c(1:61))
# anova_kappa2 <- ezANOVA(Kappa~Modelo, data = matriz_anova_kappa, wid= y, )
```
...
A continuación hacemos el __contraste para la igualdad de varianzas__ del AUC. Como son 4 poblaciones __normales y dependientes__, usaremos el __test de esfericidad de Mauchly__, cuya hipótesis nula H0 establece que se cumple la condición de homocedasticidad en las muestras.

```{r, contraste homogeneidad varianzas AUC}
# por hacer
# traspuesta_auc <- t(metricas_auc)
# 
# traspuesta_auc
# 
# traspuesta_2 <- cbind2(traspuesta_auc, c(1,2,3,4))
# 
# View(traspuesta_2)
# 
# traspuesta_2 <- as.data.frame(traspuesta_2)
# 
# colnames(traspuesta_2)[16] <- "modelo"
# 
# colnames(traspuesta_2)
# 
# mauchly.test(lm(modelo ~ ., data = traspuesta_2, na.action = na.exclude), X = ~1)
# 
# lm(modelo ~ ., data = traspuesta_2, na.action = na.exclude)
# 
# 
# ?lm
# 
