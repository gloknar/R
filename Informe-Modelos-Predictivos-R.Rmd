...
# Comprobamos que las columnas están bien formateadas. Por legibilidad, solo mostramos las 15 últimas.
tail(sapply(dataset_escalado, class), n= 15L)
```
...
control_entrenamiento_J48_MLP <- trainControl(method = "repeatedcv",
                           number=5,
                           repeats = 3,
                           seeds = NULL, #Poner "semillas_J48_MLP" cuando acabe
                           returnResamp = "final",
                           allowParallel = TRUE,
                           classProbs = TRUE,
...
```{r, Modelo J48 con 8 predictores, warning=F}
set.seed(1)
...
```{r, Modelo J48 con 4 predictores, warning=F}
set.seed(1)
...
control_entrenamiento_RegLog <- trainControl(method = "repeatedcv",
                           number=5,
                           repeats = 3,
                           seeds = NULL, # Usar "semillas_RegLog" cuando acabe
                           returnResamp = "final",
                           allowParallel = TRUE,
                           classProbs = TRUE,
                           summaryFunction = multiClassSummary, 
...
boxplot(datos_muestreo$values[,-1], col = "aquamarine")
